...
{
    public partial class frm_DS_DispatchingOrders : frm_Base
    {
        //-----------------------------------------------------------

        #region Member
        private api_TaiSan_XuatLL _defaultDataService;
        ...
        #region Contructor
        public frm_DS_DispatchingOrders()
        {
            InitializeComponent();
        ...
        private void Frm_DS_NhapTaiSan_Load(object sender, EventArgs e)
        {
            ...
            _defaultDataService = new api_TaiSan_XuatLL();
            ...
            try
            {
                ...
                var _theardRun = new Thread(new ThreadStart(() =>
                {
                    try
                    {
                        ...
                        {
                            _defaultData = _defaultDataService.Get_DispatchingList(chkAll.Checked, dtiFromDate.Value, dtiToDate.Value, cls_sys_LoaiDanhMuc.nhaCungCap_MaLoai);
        ...
        private void DeleteItemSelected()
        {
            if(dgvGrid.CurrentRow != null)
            {
                ...
                if(_itemRow != null)
                {
                    if(Helper.ConvertSToDec(""+ _itemRow["TotalReceived"]) == 0 && !Helper.ConvertSToBool(""+ _itemRow[cls_TaiSan_XuatLL.col_TrangThai] ))
                    {
                        if (TA_MessageBox.MessageBox.Show($"Bạn có chắc chắn muốn xóa phiếu {_itemRow[cls_TaiSan_XuatLL.col_SoChungTu]} không?", TA_MessageBox.MessageIcon.Question) == DialogResult.Yes)
                        {
                            _resultInfo = _defaultDataService.Delete(_itemRow[cls_TaiSan_XuatLL.col_XuatID]);
        ...
        private void OpenEditForm()
        {
            ...
            if (_itemRow != null)
            {
                frm_DispatchingOrders frm = new frm_DispatchingOrders();
                frm.SetData(""+_itemRow[cls_TaiSan_XuatLL.col_XuatID], "" + _itemRow[cls_TaiSan_XuatLL.col_SoChungTu]);
                frm.ShowDialog();
                Synchronized();
            }
