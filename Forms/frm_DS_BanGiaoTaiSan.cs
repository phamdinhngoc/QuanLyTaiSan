...
{
    public partial class frm_DS_BanGiaoTaiSan : frm_Base
    {
        //-----------------------------------------------------------

        #region Member
        private api_TaiSan_BanGiaoLL _defaultDataService;
        ...
        private void Frm_DS_NhapTaiSan_Load(object sender, EventArgs e)
        {
            ...
            _defaultDataService = new api_TaiSan_BanGiaoLL();
            ...
            try
            {
                ...
                var _theardRun = new Thread(new ThreadStart(() =>
                {
                    try
                    {
                        ...
                        {
                            _defaultData = _defaultDataService.Get_List(chkAll.Checked, dtiFromDate.Value, dtiToDate.Value);
        ...
        private void DeleteItemSelected()
        {
            if(dgvGrid.CurrentRow != null)
            {
                ...
                if(_itemRow != null)
                {
                    if(Helper.ConvertSToDec(""+ _itemRow["TotalItem"]) == 0 && !Helper.ConvertSToBool(""+ _itemRow[cls_TaiSan_BanGiaoLL.col_TrangThai] ))
                    {
                        if (TA_MessageBox.MessageBox.Show($"Bạn có chắc chắn muốn xóa phiếu {_itemRow[cls_TaiSan_BanGiaoLL.col_SoChungTu]} không?", TA_MessageBox.MessageIcon.Question) == DialogResult.Yes)
                        {
                            _resultInfo = _defaultDataService.Delete(_itemRow[cls_TaiSan_BanGiaoLL.col_BGID]);
        ...
        private void OpenEditForm()
        {
            ...
            if (_itemRow != null)
            {
                frm_BanGiaoTaiSan frm = new frm_BanGiaoTaiSan();
                frm.SetData(""+_itemRow[cls_TaiSan_BanGiaoLL.col_BGID], "" + _itemRow[cls_TaiSan_BanGiaoLL.col_SoChungTu]);
                frm.ShowDialog();
                Synchronized();
            }
